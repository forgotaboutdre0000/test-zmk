...
app/
  ...
  use_cases/
    ...
    pdf_zmk2.py        — логика pdf-zmk2 full
    ves.py             — логика ves
...
Единственная команда для обработки чертежей PDF_ZMK2. Тот же strict pipeline что и pdf-zmk.

```bash
# macOS/Linux
python3 -m app.cli pdf-zmk2 full --input-dir <dir> --sheet-write

# Windows
py -m app.cli pdf-zmk2 full --input-dir <dir> --sheet-write
```
...
## Strict pipeline (pdf-zmk и pdf-zmk2)

`pdf-zmk full` и `pdf-zmk2 full` работают по одному strict pipeline в 4 фазы. Ни одна не может быть пропущена.

...
## Purpose
Process drawings/images of type `PDF_ZMK2` (спецификации стали), normalize extracted rows, write data to Google Sheet `PDF_ZMK2`.
...
- The **Cursor agent must**:
  ...
  3. Extract every row: `elem`, `det`, `t`, `n`, `section`, `length`, `mass_sht`, `mass_total`, `mass_elem`, `steel`, `note`.
...
2. Build rows for `PDF_ZMK2` (11 columns: отпр.элем / дет / т / н / сечение / длина / шт / общ / элем / марка стали / примечания).
3. Merge all drawings into a single values table.
4. Write to Google Sheet `PDF_ZMK2` (if `--sheet-write`).
...
```
1. agent runs:  python -m app.cli pdf-zmk2 full --input-dir <dir> --dry-run
...
6. agent runs:  python -m app.cli pdf-zmk2 full --input-dir <dir> --sheet-write
